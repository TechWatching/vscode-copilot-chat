	...
	private async *convertStreamToApiChoices(resp: ResponseStream, finishedCb: FinishedCallback, baseTelemetryData: TelemetryWithExp): AsyncIterable<APIChoice> {
		...
		for await (const chunk of resp.stream) {
			for (let i = 0; i < chunk.choices.length; i++) {
				...
				const solutionDecision = finishedCb(completion.accumulator.responseSoFar, {
					index: chunkIdx,
					text: chunkText ?? '',
					finished: hasFinishReason,
				} satisfies RequestDelta);

				if (hasFinishReason || solutionDecision !== undefined) {
					// mark as finished
					completion.isFinished = true;

					const completionText = solutionDecision === undefined || typeof solutionDecision !== 'number'
						? completion.accumulator.responseSoFar
						: completion.accumulator.responseSoFar.slice(0, solutionDecision);

					yield createAPIChoice(
						chunkIdx,
						completionText,
						chunk.choices[i].finish_reason ?? '',
